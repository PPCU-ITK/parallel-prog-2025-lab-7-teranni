...
int main(int argc, const char** argv)
{
  ...
  #pragma omp target data map(A[0:((imax+2) * (jmax+2))]) map(Anew[0:((imax+2) * (jmax+2))])
  {
    while ( error > tol && iter < iter_max )
    {
      ...
      // #pragma omp target distribute parallel for collapse(2) reduction(max:error)
      ...
      for( int i = 1; i < imax+1; i++) 
      {
        ...
        {
          Anew[(j)*(imax+2)+i] = 0.25f * ( A[(j+1)*(imax+2)+i] + A[(j)*(imax+2)+i-1]
              + A[(j-1)*(imax+2)+i] + A[(j)*(imax+2)+i+1]);
      ...
      #pragma omp target teams distribute parallel for // map(A[0:(imax+2)*(jmax+2)]) map(Anew[0:(imax+2)*(jmax+2)]) collapse(2)
      for( int i = 1; i < imax+1; i++)
      {
        for( int j = 1; j < jmax+1; j++ )
        {
