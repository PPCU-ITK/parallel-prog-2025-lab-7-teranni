...
#pragma omp target data map(alloc:rho[0:total_size]) map(alloc:rhou[0:total_size]) map(alloc:rhov[0:total_size]) map(alloc:E[0:total_size]) \
    map(alloc:rho_new[0:total_size]) map(alloc:rhou_new[0:total_size]) map(alloc:rhov_new[0:total_size]) map(alloc:E_new[0:total_size]) \
    map(alloc:solid[0:total_size])
{
    ...
    cout << "Time: " << chrono::duration_cast<chrono::milliseconds>(chrono::high_resolution_clock::now() - t2).count() << " ms" << endl;
}
    cout << "Time with allocations: " << chrono::duration_cast<chrono::milliseconds>(chrono::high_resolution_clock::now() - t1).count() << " ms" << endl;
